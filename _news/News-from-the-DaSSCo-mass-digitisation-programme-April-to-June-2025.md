...
---
## Image: The whole Alice setup at NHMD
...
The DaSSCo IT Team has also made significant progress on the new Ingestion Client. The new version represents a major leap forward, solving many issues from the previous version. The updated interface is more intuitive and visually clearer, reducing user errors such as selecting incorrect fields. Its efficiency has also improved. The client now supports crash recovery, so if an upload stops at e.g. 56 % completion, the process can simply be restarted and will resume from the same point. This has greatly reduced lost time and improved the digitisation workflow.
